...
import { departmentLabel, employeeTypeLabel, DEPARTMENT_OPTIONS, EMPLOYEE_TYPE_OPTIONS } from '../../shared/constants/labels';
...
export default function ExtraInfoCard({ data = {}, userType, editable = [], onSubmit }) {
  ...
  const fields = useMemo(() => {
    ...
    if (userType === "employee") {
      return [
        { key: "department",     label: "Τμήμα",        type: "select", options: DEPARTMENT_OPTIONS },
        { key: "employee_type",  label: "Είδος απασχόλησης", type: "select", options: EMPLOYEE_TYPE_OPTIONS },
        { key: "has_vehicle",    label: "Διαθέτει όχημα", type: "boolean" },
      ];
    }
    return [];
      ...
      {!edit ? (
        <div className="divide-y px-3 pb-2">
          {fields.map(f => {
            ...
            let value = raw;
            if (f.key === 'department') value = departmentLabel(raw);
            if (f.key === 'employee_type') value = employeeTypeLabel(raw);
            return (
